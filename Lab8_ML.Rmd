...
One of the problems that is going to occur this time is R and Bioconductor have been updated since last lab. We are going to do as we said and keep the old versions, but you will see warnings, for the time being, ignore them, we force BiocManager to install the older version by specifying it :

```{r warning=FALSE, message=FALSE}
pkgs_needed = c("MASS","ExperimentHub", "tidyverse","glmnet",
                "RColorBrewer","caret", "magrittr", "e1071")
letsinstall = setdiff(pkgs_needed, installed.packages()) 
if (length(letsinstall) > 0) {
  BiocManager::install(setdiff(pkgs_needed, installed.packages()),version="3.12")
}
```
...
The prediction error is almost half compared to that  with only the 
insulin and glufast variables. 

Is this because indeed the classifier based on
5 variables is better, or are we just overfitting our dataset? 
...
as follows. We use the package `glmnet` to fit this model. `glmnet` fits this model
with a regularization penalty. When using an unregularized objective, we choose our parameters, $\beta$, to minimize the error of our classifier on the training data. But regularization introduces another term to the model objective, namely, a penalty of the form $\lambda ||beta||$ where $\lambda$ is a hyperparameter that we have to pick ourselves. The LASSO penalty, which we explore here, penalizes the L1-norm of the parameter vector, aka the sum of the absolute values of the coefficients. 

**Quiz Question 1**: Does a larger choice for $\lambda$ improve or harm the training set accuracy of the classifier?

But now we are left with a problem. How do we pick the correct value of this hyperparameter, $\lambda$? Different choices of the penalty correspond to different points on the bias-variance frontier. Larger values of $\lambda$ produce simpler (more biased) models, but improve the stability (reduce the variance) of the model fitting procedure. 
...
**Quiz questions 2 and 3**: What is the true positive rate (TPR) and true
...
**Quiz question 4**: Inspect the `glmfit` object: For how many different 
values of $\lambda$ did `glmnet` fit the model using settings as above?


**Quiz question 5**: For each of the values of $\lambda$ in the object above, 
calculate the L1 Norm (as we did above for two values of $\lambda$). 
Which $\lambda$ most closely corresponds to a L1 norm 6000?


**Quiz question 6**: How many non-zero coefficients do you get for 
...
**Quiz question 7**: How does the confusion table look like for $\lambda=$ 
...
**Quiz question 8**: What features drive the classification (at the $\lambda$ 
chosen by cross-validation)? Report the feature with the largest associated coefficient. Hint: The first line in the code below might be useful to get you started.
...
The methods we've seen so far in this lab are all linear, i.e. they assume that 
we can separate our classes using a linear function of the predictor variables. 
This simplifying assumption can be helpful for the same reason that we applied
regularization above. Simpler linear models trade off increased bias for decreased
variance. 

Most of the nonlinear methods that we might use, such as kernelized support vector machines (see package `e1071`), random forests (`RandomForest`), boosting methods (`mboost`), have interfaces quite similar to `glmnet`. 

```{r forest_fit, warning=FALSE}
library(e1071)
svmfit = svm(x = t(exprs(zellerNC)),
                y = factor(zellerNC$disease))

pred = predict(svmfit, newx = t(exprs(zellerNC)))

confusion_table = table(predicted = pred, truth = zellerNC$disease)
confusion_table
```

**Quiz Question 9**: How could we modify the chunk above to estimate the performance of the SVM model on unseen data? [multiple choice]
